    ...
    <div className="group relative flex flex-col h-full bg-white border border-purple-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      ...
      {isOwner && (
        <button
          ...
          className="absolute top-4 right-4 p-2 bg-white/80 backdrop-blur-md rounded-full text-purple-600 hover:text-red-600 hover:bg-red-50 transition-colors z-10 shadow-sm"
          title="Delete Gig"
        >
          <Trash2 size={18} />
        </button>
      ...
      <div className="p-6 flex flex-col grow">
        ...
        {/* Assigned Warning Overlay */}
        {isAssigned && (
            <p className="mt-4 text-xs font-medium text-red-500 italic">
              {user && gig.ownerId && user._id === gig.ownerId._id
                ? "You Hired freelancer for this Gig"
                : "This Gig is already assigned you can only view bids"}
            </p>
